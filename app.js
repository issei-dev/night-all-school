// ナイト・オール・スクール
// 統合版 app.js v17
// - ガチャ完成版（比較表示 / 演出 / 一括売却 / ロック / お気に入り）
// - 戦闘 v1.2（ターゲットUI強化 / ダメージ数字 / 被弾演出 / 必殺演出 / 勝利演出）

const STORAGE_KEY = 'night-all-school-integrated-v17';
...
function sanitizeProtectedWeapons() {
  ...
  weaponState.favoriteWeapons = nextFavorites;
}
...
function getEquippedWeaponUsageMap() {
  const map = new Map();
  Object.values(formationState.equippedWeapons).forEach((weaponId) => { if (!weaponId) return; map.set(weaponId, (map.get(weaponId) || 0) + 1); });
  return map;
}
...
function sanitizeEquippedWeapons() {
  ...
  Object.keys(formationState.equippedWeapons).forEach((slotId) => {
    ...
    if (!canEquipWeaponToSlot(weapon, slotId)) formationState.equippedWeapons[slotId] = '';
  });
...
function getEffectiveWeaponStats(weapon) {
  if (!weapon) return { hp: 0, atk: 0, level: 0 };
  const level = getWeaponEnhanceLevel(weapon.id);
  const rate = 1 + (level * WEAPON_ENHANCE_RATE);
  return { hp: Math.floor((weapon.baseStats?.hp ?? 0) * rate), atk: Math.floor((weapon.baseStats?.atk ?? 0) * rate), level };
}
function canEnhanceWeapon(weaponId) {
  const weapon = getWeaponById(weaponId);
  if (!weapon) return false;
  const level = getWeaponEnhanceLevel(weaponId);
  return getOwnedWeaponCount(weaponId) > 0 && level < WEAPON_ENHANCE_MAX && resourceState.materialCore >= getWeaponEnhanceCost(level);
}
function tryEnhanceWeapon(weaponId) {
  const weapon = getWeaponById(weaponId);
  if (!weapon) return false;
  const level = getWeaponEnhanceLevel(weaponId);
  if (level >= WEAPON_ENHANCE_MAX) { alert('最大強化済みです。'); return false; }
  const cost = getWeaponEnhanceCost(level);
  if (resourceState.materialCore < cost) { alert(`マテリアルコアが不足しています。必要数：${cost}`); return false; }
  resourceState.materialCore -= cost;
  weaponState.enhancements[weaponId] = level + 1;
  saveGameState(); rerenderAllSections(); alert(`${weapon.name} を +${level + 1} に強化しました。`); return true;
}
function equipWeaponToSlot(slotId, weaponId) {
  ...
  sanitizeEquippedWeapons(); saveGameState(); rerenderAllSections(); return true;
}
function trySellDuplicateWeapon(weaponId) {
  ...
  resourceState.materialCore += getWeaponSellValue(weapon);
  saveGameState(); rerenderAllSections(); alert(`${weapon.name} を売却しました。`); showScreen('gacha');
}
function getBulkSellTargets(options = {}) {
  const rarity = options.rarity || null; const targets = [];
  getWeaponsList().forEach((weapon) => {
    if (rarity && (weapon.rarity || 1) !== rarity) return;
    const sellable = getSellableWeaponCount(weapon.id);
    if (sellable > 0) targets.push({ weapon, count: sellable, totalGain: sellable * getWeaponSellValue(weapon) });
  });
  return targets;
}
function executeBulkSell(options = {}) {
  ...
  saveGameState(); rerenderAllSections(); alert(`${options.rarity === 1 ? '★1重複一括売却' : '全部売却'} を実行しました。\n売却本数：${soldCount}\n獲得コア：${gain}`); showScreen('gacha');
}
function lockHighRarityWeapons() {
  let count = 0;
  getWeaponsList().forEach((weapon) => { if ((weapon.rarity || 1) >= 2 && getOwnedWeaponCount(weapon.id) > 0 && !isWeaponLocked(weapon.id)) { setWeaponLocked(weapon.id, true); count += 1; } });
  saveGameState(); rerenderAllSections(); alert(count > 0 ? `★2以上の武器を ${count} 件ロックしました。` : 'ロック対象の★2以上武器はありません。'); showScreen('gacha');
}
function toggleFavoriteFromGachaResult(index) {
  const result = gachaState.lastResults[index]; if (!result) return;
  const next = !isWeaponFavorite(result.weaponId); setWeaponFavorite(result.weaponId, next); if (next) setWeaponLocked(result.weaponId, true);
  saveGameState(); rerenderAllSections(); alert(next ? `${result.name} をお気に入り登録しました。売却保護も有効です。` : `${result.name} のお気に入りを解除しました。`); showScreen('gacha');
}
...
function buildEquipComparison(slotId, newWeaponId) {
  ...
  return {
    ...
    currentHp: currentStats.finalHp, currentAtk: currentStats.finalAtk,
    newHp: nextStats.finalHp, newAtk: nextStats.finalAtk,
...
function renderComparisonHtml(result) {
  const weapon = getWeaponById(result.weaponId); if (!weapon) return '<p class="muted">比較データなし</p>';
  const targets = [
  ...
  const rows = targets.map((target) => {
    ...
    const hpPrefix = cmp.hpDiff > 0 ? '+' : ''; const atkPrefix = cmp.atkDiff > 0 ? '+' : '';
...
function getWeightedRarity(weights) {
  ...
  for (const [rarity, weight] of Object.entries(weights)) { roll -= weight; if (roll <= 0) return Number(rarity); }
  return 1;
}
...
function createResultEntry(weapon, ownershipTracker, source, guaranteed = false) {
  ...
  ownershipTracker[weapon.id] = beforeCount + 1; addOwnedWeapon(weapon.id, 1);
...
function revealResultAtIndex(index) {
  const result = gachaState.lastResults[index]; if (!result) return;
  if (index + 1 <= gachaState.revealCount) return;
  gachaState.revealCount = index + 1; if (result.rarity >= 3) triggerRareFlash(); saveGameState(); renderGachaSection();
}
...
function runSingleGacha() {
  ...
  const picked = drawWeaponWithMinimum(1); if (!picked) { alert('ガチャ結果の生成に失敗しました。'); return; }
...
function renderEquipButtonsHtml(result, index) {
  ...
  return `<div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;width:100%;">${targets.map((target) => { const disabled = canEquipWeaponToSlot(weapon, target.slotId) ? '' : 'disabled'; return `<button class="text-button gacha-card-equip" data-gacha-equip-index="${escapeHtml(index)}" data-gacha-equip-slot="${escapeHtml(target.slotId)}" ${disabled}>${escapeHtml(target.label)} に装備</button>`; }).join('')}</div>`;
}
function renderRevealedGachaCard(result, index) {
  ...
  const canSell = getSellableWeaponCount(result.weaponId) > 0;
  const enhanceLevel = getWeaponEnhanceLevel(result.weaponId);
  const enhanceLabel = enhanceLevel >= WEAPON_ENHANCE_MAX ? '強化最大' : `強化する（${getWeaponEnhanceCost(enhanceLevel)} コア）`;
  const canEnhance = canEnhanceWeapon(result.weaponId);
  const protectedLabel = isWeaponProtected(result.weaponId) ? `<div style="font-size:11px;color:#facc15;margin-top:6px;">${isWeaponFavorite(result.weaponId) ? 'お気に入り' : 'ロック中'}</div>` : '';
  const favLabel = isWeaponFavorite(result.weaponId) ? '★ お気に入り解除' : '☆ お気に入り登録';
  const extraGlow = result.rarity === 3 ? '0 0 28px rgba(255,216,106,.55),0 8px 26px rgba(0,0,0,.35)' : result.isNew ? '0 0 20px rgba(34,197,94,.35),0 8px 20px rgba(0,0,0,.25)' : '0 8px 20px rgba(0,0,0,.25)';
  return `
    <div style="border:1px solid ${rarityColor};border-radius:16px;padding:14px;background:${getRarityBg(result.rarity)};min-height:420px;display:flex;flex-direction:column;justify-content:space-between;box-shadow:${extraGlow};animation:gachaFlipIn .45s ease;${result.isNew ? 'animation:gachaFlipIn .45s ease, gachaNewPulse 1.4s ease-in-out infinite;' : ''}">
      <div>
        <div style="display:flex;justify-content:space-between;align-items:center;gap:8px;">
          <span style="font-size:13px;color:${rarityColor};font-weight:800;">${getRarityLabel(result.rarity)}</span>
          <span style="font-size:11px;padding:4px 8px;border-radius:999px;background:${badgeBg};color:white;">${escapeHtml(result.isNew ? 'NEW' : '重複')}</span>
        </div>
        ...
        ${result.guaranteed ? '<div style="font-size:11px;color:#fde68a;margin-top:6px;">保証枠</div>' : ''}
        ${result.isNew ? '<div style="font-size:11px;color:#86efac;margin-top:6px;">NEWエフェクト</div>' : ''}
        ${protectedLabel}
...
function renderLastGachaResults() {
  ensureGachaFxStyles();
  if (!gachaState.lastResults.length) return '<p class="muted">まだ最新の結果はありません。</p>';
  const cards = gachaState.lastResults.map((result, index) => (index < gachaState.revealCount ? renderRevealedGachaCard(result, index) : renderHiddenGachaCard(result, index))).join('');
...
function equipWeaponToSlotFromGachaResult(index, slotId) { const result = gachaState.lastResults[index]; if (!result) return; const ok = equipWeaponToSlot(slotId, result.weaponId); if (!ok) return; const weapon = getWeaponById(result.weaponId); alert(`${weapon?.name || result.name} を ${getSlotDisplayName(slotId)} に装備しました。`); showScreen('gacha'); }
function enhanceWeaponFromGachaResult(index) { const result = gachaState.lastResults[index]; if (!result) return; const ok = tryEnhanceWeapon(result.weaponId); if (ok) showScreen('gacha'); }
function sellWeaponFromGachaResult(index) { const result = gachaState.lastResults[index]; if (!result) return; trySellDuplicateWeapon(result.weaponId); }
function renderOwnedWeaponsSummary() {
  const weapons = getWeaponsList().filter((weapon) => getOwnedWeaponCount(weapon.id) > 0);
  if (!weapons.length) return '<p class="muted">所持武器なし</p>';
  const grouped = { 1: [], 2: [], 3: [] };
  weapons.forEach((weapon) => { grouped[weapon.rarity || 1].push(weapon); });
  return [1, 2, 3].map((rarity) => {
    const list = grouped[rarity] || [];
    if (!list.length) return `<h4>★${rarity}</h4><p class="muted">なし</p>`;
    return `<h4>★${rarity}</h4><ul>${list.map((weapon) => { const ew = getEffectiveWeaponStats(weapon); const marks = `${isWeaponFavorite(weapon.id) ? ' / お気に入り' : ''}${isWeaponLocked(weapon.id) ? ' / ロック' : ''}`; return `<li><strong>${escapeHtml(weapon.name)}</strong> <span class="muted">[${escapeHtml(getWeaponTypeDisplay(weapon))} / 所持 ${escapeHtml(getOwnedWeaponCount(weapon.id))} / 売却可 ${escapeHtml(getSellableWeaponCount(weapon.id))} / +${escapeHtml(ew.level)}${escapeHtml(marks)}]</span><br><span class="muted">HP ${escapeHtml(ew.hp)} / ATK ${escapeHtml(ew.atk)}</span></li>`; }).join('')}</ul>`;
  }).join('');
}
function attachGachaEvents() {
  ...
  document.querySelectorAll('[data-gacha-equip-index]').forEach((button) => { const index = Number(button.getAttribute('data-gacha-equip-index')); const slotId = button.getAttribute('data-gacha-equip-slot'); button.addEventListener('click', () => equipWeaponToSlotFromGachaResult(index, slotId)); });
  document.querySelectorAll('[data-gacha-enhance-index]').forEach((button) => button.addEventListener('click', () => enhanceWeaponFromGachaResult(Number(button.getAttribute('data-gacha-enhance-index')))));
  ...
  document.querySelectorAll('[data-gacha-sell-index]').forEach((button) => button.addEventListener('click', () => sellWeaponFromGachaResult(Number(button.getAttribute('data-gacha-sell-index')))));
}
function renderGachaSection() {
  const screen = document.getElementById('screen-gacha'); if (!screen || !gameData.weapons) return;
  const container = screen.querySelector('.screen-inner'); if (!container) return;
...
function cleanBattleTransientEffects(runtime) {
  if (!runtime) return;
  const now = Date.now();
  [...runtime.party, ...runtime.enemies].forEach((unit) => {
    unit.damagePopups = (unit.damagePopups || []).filter((item) => now - item.ts < 900);
    if (unit.hitFlashUntil && now > unit.hitFlashUntil) unit.hitFlashUntil = 0;
    if (unit.defeatedFlashUntil && now > unit.defeatedFlashUntil) unit.defeatedFlashUntil = 0;
  });
  if (runtime.burstFx && now > runtime.burstFx.until) runtime.burstFx = null;
  if (runtime.victoryFx && now > runtime.victoryFx.until) runtime.victoryFx = null;
}
function pushDamagePopup(unit, value, kind = 'damage') {
  if (!unit) return;
  if (!unit.damagePopups) unit.damagePopups = [];
  unit.damagePopups.push({ value, kind, ts: Date.now() });
  unit.hitFlashUntil = Date.now() + 320;
  if (!unit.alive) unit.defeatedFlashUntil = Date.now() + 650;
}
function setBurstFx(runtime, text, rare = false) {
  runtime.burstFx = { text, rare, until: Date.now() + 950 };
}
function setVictoryFx(runtime, text) {
  runtime.victoryFx = { text, until: Date.now() + 1600 };
}
function createBattleRuntime(battle) {
  const runtime = {
    party: [createPlayerUnit('protagonist'), createPlayerUnit('char_towa'), createPlayerUnit('char_hinano'), createPlayerUnit('char_suzu')],
    enemies: (battle.enemyGroup || []).map(createEnemyUnit),
    turnBuffs: { attackBoost: 0, defenseReduce: 0 },
    burstFx: null,
    victoryFx: null
  };
  initializePartyCommands(runtime.enemies);
  return runtime;
}
...
function applyDamage(target, amount) {
  ...
  const actual = Math.max(1, Math.min(target.hp, Math.floor(amount)));
  target.hp = Math.max(0, target.hp - actual);
  if (target.hp <= 0) { target.hp = 0; target.alive = false; }
  pushDamagePopup(target, actual, 'damage');
  return actual;
}
...
function applyProtagonistCommand(runtime, context, logs) {
  const cmd = battleUiState.protagonistCommand;
  if (cmd === 'attack-order') { context.atkBoostRate += COMMAND_RULES.attackBoost; logs.push(`主人公の攻撃指示：味方ATKが ${Math.round(COMMAND_RULES.attackBoost * 100)}% 上昇。`); }
  else if (cmd === 'defense-order') { context.incomingReduceRate += COMMAND_RULES.defenseReduce; logs.push(`主人公の防御指示：味方被ダメージ ${Math.round(COMMAND_RULES.defenseReduce * 100)}% 軽減。`); }
  else if (cmd === 'focus-order') { runtime.party.filter((unit) => unit.alive && unit.id !== 'protagonist').forEach((unit) => addGauge(unit, COMMAND_RULES.focusGauge)); logs.push(`主人公の集中指示：味方必殺ゲージ +${COMMAND_RULES.focusGauge}。`); }
  else { logs.push('主人公は待機した。'); }
}
function calcPlayerDamage(attacker, multiplier, context) { return Math.max(1, Math.floor(attacker.atk * multiplier * (1 + (context.atkBoostRate || 0)))); }
function calcEnemyDamage(attacker, multiplier, target, context) { let dmg = Math.max(1, Math.floor(attacker.atk * multiplier)); if (target.guarding) dmg = Math.max(1, Math.floor(dmg * (1 - COMMAND_RULES.guardReduction))); if ((context.incomingReduceRate || 0) > 0) dmg = Math.max(1, Math.floor(dmg * (1 - context.incomingReduceRate))); return dmg; }
function chooseEnemyTarget(runtime, enemy) {
  const aliveParty = getAliveUnits(runtime.party);
  if (!aliveParty.length) return null;
  const taunter = aliveParty.find((unit) => unit.taunt > 0);
  if (taunter) return taunter;
  const mode = enemy.combat?.ai?.targeting || 'front';
  if (mode === 'lowest_hp') return aliveParty.slice().sort((a, b) => a.hp - b.hp)[0];
  return aliveParty[0];
}
function chooseEnemyAction(enemy) {
  if ((enemy.gauge || 0) >= (enemy.combat?.burst?.gaugeCost || 100)) return { type: 'burst', name: enemy.combat?.burst?.name || '必殺技', multiplier: enemy.combat?.burst?.multiplier || 2.0 };
  if ((enemy.skillCooldown || 0) <= 0 && Math.random() < (enemy.combat?.ai?.skillChance || 0.35)) return { type: 'skill', name: enemy.combat?.skill?.name || 'スキル', multiplier: enemy.combat?.skill?.multiplier || 1.2, cooldown: enemy.combat?.skill?.cooldown || 2 };
  return { type: 'attack', name: enemy.combat?.normalAttackName || '通常攻撃', multiplier: 1.0 };
}
function getSelectedTarget(runtime, actorId) {
  const selectedId = battleUiState.partyCommands[actorId]?.targetId || '';
  const enemy = getUnitById(runtime.enemies, selectedId);
  if (enemy && enemy.alive) return enemy;
  return getFirstAlive(runtime.enemies);
}
function executeSingleHit(actor, target, damage, logs, label) {
  const actual = applyDamage(target, damage);
  logs.push(`${actor.name} の${label}！ ${target.name} に ${actual} ダメージ。`);
  if (!target.alive) logs.push(`${target.name} を撃破した。`);
}
function executePlayerActions(runtime, context, logs) {
  ['char_towa', 'char_hinano', 'char_suzu'].forEach((id) => {
    ...
    if (id === 'char_hinano' && (command.action === 'skill' || command.action === 'burst')) {
      ...
      targets.forEach((target) => { const damage = calcPlayerDamage(actor, multiplier, context); executeSingleHit(actor, target, damage, logs, command.action === 'burst' ? `必殺技「${actor.combat?.burst?.name || '必殺技'}」` : `スキル「${actor.combat?.skill?.name || 'スキル'}」`); });
      if (command.action === 'burst') actor.gauge = 0; else addGauge(actor, actor.combat?.skill?.gaugeGain ?? GAUGE_RULES.skillUse);
      return;
    }
    const target = getSelectedTarget(runtime, id); if (!target) return;
    if (id === 'char_suzu' && command.action === 'skill') {
      const damage = calcPlayerDamage(actor, actor.combat?.skill?.multiplier || 1.1, context); executeSingleHit(actor, target, damage, logs, `スキル「${actor.combat?.skill?.name || 'スキル'}」`); actor.taunt = actor.combat?.skill?.tauntTurns || 2; actor.guarding = true; addGauge(actor, actor.combat?.skill?.gaugeGain ?? GAUGE_RULES.skillUse); logs.push(`${actor.name} は挑発状態になった。敵の攻撃を引きつける。`); return;
    }
    if (id === 'char_suzu' && command.action === 'burst') {
      setBurstFx(runtime, `${actor.name} ${actor.combat?.burst?.name || '必殺技'}！`, true);
      const damage = calcPlayerDamage(actor, actor.combat?.burst?.multiplier || 1.95, context); executeSingleHit(actor, target, damage, logs, `必殺技「${actor.combat?.burst?.name || '必殺技'}」`); actor.taunt = actor.combat?.burst?.tauntTurns || 2; actor.gauge = 0; logs.push(`${actor.name} は必殺と同時に挑発状態になった。`); return;
    }
    if (command.action === 'skill') { const damage = calcPlayerDamage(actor, actor.combat?.skill?.multiplier || 1.25, context); executeSingleHit(actor, target, damage, logs, `スキル「${actor.combat?.skill?.name || 'スキル'}」`); addGauge(actor, actor.combat?.skill?.gaugeGain ?? GAUGE_RULES.skillUse); return; }
    if (command.action === 'burst') { setBurstFx(runtime, `${actor.name} ${actor.combat?.burst?.name || '必殺技'}！`, true); const damage = calcPlayerDamage(actor, actor.combat?.burst?.multiplier || 2.0, context); executeSingleHit(actor, target, damage, logs, `必殺技「${actor.combat?.burst?.name || '必殺技'}」`); actor.gauge = 0; return; }
    const damage = calcPlayerDamage(actor, 1.0, context); executeSingleHit(actor, target, damage, logs, `通常攻撃「${actor.combat?.normalAttackName || '通常攻撃'}」`); addGauge(actor, GAUGE_RULES.normalAttack);
  });
}
function executeEnemyActions(runtime, context, logs) {
  runtime.enemies.forEach((enemy) => {
    if (!enemy.alive) return;
    const target = chooseEnemyTarget(runtime, enemy); if (!target) return;
    const choice = chooseEnemyAction(enemy);
    if (choice.type === 'burst') setBurstFx(runtime, `${enemy.name} ${choice.name}！`, true);
    const damage = calcEnemyDamage(enemy, choice.multiplier, target, context);
    const actual = applyDamage(target, damage);
    logs.push(`${enemy.name} の${choice.type === 'attack' ? '通常攻撃' : choice.type === 'skill' ? 'スキル' : '必殺技'}「${choice.name}」！ ${target.name} に ${actual} ダメージ。`);
    if (choice.type === 'attack') addGauge(enemy, GAUGE_RULES.normalAttack);
    if (choice.type === 'skill') { addGauge(enemy, GAUGE_RULES.skillUse); enemy.skillCooldown = choice.cooldown || 2; }
    if (choice.type === 'burst') enemy.gauge = 0;
    if (target.alive) addGauge(target, GAUGE_RULES.damageTaken);
    if (target.guarding) logs.push(`${target.name} は防御中のためダメージを大きく軽減した。`);
...
function buildBattleRewardResult(battle, status) {
  if (status !== 'victory') return { status: 'defeat', title: '敗北', lines: ['報酬はありません。装備や行動を見直して再挑戦しましょう。'], rewards: { materialCore: 0, exp: 0 } };
  const newlyCleared = !progressState.clearedBattles.includes(battle.id);
  let materialCore = 0, exp = 0; const lines = [];
  if (newlyCleared) { materialCore += battle.firstClearReward?.materialCore ?? 0; exp += battle.firstClearReward?.exp ?? 0; progressState.clearedBattles.push(battle.id); lines.push(`初回クリア報酬：マテリアルコア ${battle.firstClearReward?.materialCore ?? 0} / EXP ${battle.firstClearReward?.exp ?? 0}`); }
  else { exp += battle.repeatReward?.exp ?? 0; lines.push(`再挑戦報酬：EXP ${battle.repeatReward?.exp ?? 0}`); }
  addRewards(materialCore, exp); saveGameState(); lines.push(`現在の所持数：マテリアルコア ${resourceState.materialCore} / EXP ${resourceState.exp}`);
  return { status: 'victory', title: '勝利', lines, rewards: { materialCore, exp } };
}
function executeBattleTurn() {
  const battle = getBattleById(battleUiState.currentBattleId); const runtime = battleUiState.runtime;
  if (!battle || !runtime || battleUiState.result) return;
  battleUiState.turnCount += 1;
  tickStatuses(runtime); cleanBattleTransientEffects(runtime);
  const logs = [`--- ターン ${battleUiState.turnCount} ---`];
  const context = { atkBoostRate: 0, incomingReduceRate: 0 };
...
function renderDamagePopups(unit) {
  const popups = (unit.damagePopups || []).slice(-2);
  if (!popups.length) return '';
  return `<div style="position:absolute;inset:0;pointer-events:none;overflow:visible;">${popups.map((popup, index) => `<div style="position:absolute;right:12px;top:${8 + (index * 18)}px;font-weight:800;color:${popup.kind === 'heal' ? '#86efac' : '#fca5a5'};text-shadow:0 0 8px rgba(0,0,0,.45);animation:battleDamagePop .85s ease-out both;">-${escapeHtml(popup.value)}</div>`).join('')}</div>`;
}
function renderEnemyTargetBadge(unit) {
  const isSelected = battleUiState.targetPickerMember && battleUiState.partyCommands[battleUiState.targetPickerMember]?.targetId === unit.id;
  return isSelected ? `<div style="position:absolute;top:10px;left:10px;padding:4px 8px;border-radius:999px;background:#38bdf8;color:#07203a;font-size:11px;font-weight:800;">選択中</div>` : '';
}
function renderBattleBurstFx(runtime) {
  if (!runtime?.burstFx) return '';
  return `<div style="position:sticky;top:0;z-index:5;margin-bottom:8px;padding:12px 14px;border-radius:16px;background:linear-gradient(90deg, rgba(255,216,106,.28), rgba(248,113,113,.18));border:1px solid rgba(255,216,106,.45);text-align:center;font-weight:800;letter-spacing:.04em;animation:battleBurstGlow .9s ease-out both;">${escapeHtml(runtime.burstFx.text)}</div>`;
}
function renderBattleVictoryFx(runtime) {
  if (!runtime?.victoryFx) return '';
...
function renderBattleUnitBox(unit, isEnemy = false) {
  const gaugeColor = isEnemy ? '#f87171' : '#60a5fa';
  const hitStyle = unit.hitFlashUntil && Date.now() < unit.hitFlashUntil ? 'animation:battleHitShake .28s ease;' : '';
  const selectedOutline = isEnemy && battleUiState.targetPickerMember && battleUiState.partyCommands[battleUiState.targetPickerMember]?.targetId === unit.id ? 'box-shadow:0 0 0 2px #38bdf8, 0 10px 24px rgba(0,0,0,.25);border-color:#38bdf8;' : '';
  ...
  return `
    <div class="info-card battle-unit-card" ${clickAttr} style="position:relative;margin-top:10px;opacity:${unit.alive ? 1 : 0.58};cursor:${isEnemy && unit.alive ? 'pointer' : 'default'};${hitStyle}${selectedOutline}${defeatedStyle}">
      ${isEnemy ? renderEnemyTargetBadge(unit) : ''}
      <h5 style="margin:0 0 8px;">${escapeHtml(unit.name)}</h5>
      <p class="muted" style="margin:0;">HP ${escapeHtml(unit.hp)} / ${escapeHtml(unit.maxHp)} / ATK ${escapeHtml(unit.atk)}</p>
      <p class="muted" style="margin:6px 0 0;">ゲージ ${escapeHtml(unit.gauge)} / 100 ${unit.guarding && !isEnemy ? '/ 防御中' : ''} ${unit.taunt > 0 ? '/ 挑発' : ''}</p>
      <div style="height:10px;border-radius:999px;background:#1f2c54;margin-top:8px;overflow:hidden;${isEnemy ? 'outline:1px solid rgba(255,255,255,.05);' : ''}"><div style="height:100%;width:${escapeHtml(Math.max(0, Math.min(100, (unit.hp / Math.max(1, unit.maxHp)) * 100)))}%;background:${isEnemy ? 'linear-gradient(90deg,#fb7185,#f43f5e)' : 'linear-gradient(90deg,#60a5fa,#38bdf8)'};"></div></div>
      ${renderDamagePopups(unit)}
      ${isEnemy && unit.alive ? '<div style="margin-top:8px;font-size:11px;color:#94a3b8;">HPバークリックでターゲット設定</div>' : ''}
    </div>
  `;
}
function renderTargetAssist(memberId) {
  const enemies = getTargetableEnemies();
  if (!enemies.length) return '<p class="muted">敵なし</p>';
  const targetId = battleUiState.partyCommands[memberId]?.targetId || enemies[0]?.id || '';
  const targetName = getUnitById(battleUiState.runtime?.enemies || [], targetId)?.name || '未選択';
  const active = battleUiState.targetPickerMember === memberId;
  return `<div style="margin-top:8px;"><button class="text-button battle-pick-target ${active ? 'is-selected' : ''}" data-pick-target="${escapeHtml(memberId)}">${escapeHtml(active ? 'ターゲット指定中' : 'このキャラのターゲットを選ぶ')}</button><p class="muted" style="margin-top:8px;">現在ターゲット：${escapeHtml(targetName)}</p></div>`;
}
function renderBattleCommandControls(runtime) {
  const protagonistButtons = `
    <div class="button-group" style="max-width:none;flex-direction:row;flex-wrap:wrap;">
      <button class="text-button battle-protagonist-command ${battleUiState.protagonistCommand === 'attack-order' ? 'is-selected' : ''}" data-protagonist-command="attack-order">攻撃指示 (+5%)</button>
      <button class="text-button battle-protagonist-command ${battleUiState.protagonistCommand === 'defense-order' ? 'is-selected' : ''}" data-protagonist-command="defense-order">防御指示 (10%軽減)</button>
      <button class="text-button battle-protagonist-command ${battleUiState.protagonistCommand === 'focus-order' ? 'is-selected' : ''}" data-protagonist-command="focus-order">集中指示 (ゲージ+5)</button>
      <button class="text-button battle-protagonist-command ${battleUiState.protagonistCommand === 'wait' ? 'is-selected' : ''}" data-protagonist-command="wait">待機</button>
    </div>
  `;
  const members = getAliveUnits(runtime.party.filter((unit) => unit.id !== 'protagonist'));
  return `
    <div class="info-card" style="margin-top:12px;">
      <h4 style="margin-top:0;">主人公コマンド</h4>
      ${protagonistButtons}
      <p class="muted" style="margin-top:10px;">通常攻撃 +20 / スキル +5 / 被ダメ +10 / 防御 +10 / 100で必殺可</p>
    </div>
    ${members.map((unit) => {
      const cmd = battleUiState.partyCommands[unit.id] || { action: 'attack', targetId: '' };
      return `
        <div class="info-card" style="margin-top:12px;">
          <h4 style="margin-top:0;">${escapeHtml(unit.name)} の行動</h4>
          <p class="muted">役割：${escapeHtml(unit.combat?.role || '-')} / ゲージ ${escapeHtml(unit.gauge)} / 100</p>
          <div class="button-group" style="max-width:none;flex-direction:row;flex-wrap:wrap;">
            <button class="text-button battle-party-command ${cmd.action === 'attack' ? 'is-selected' : ''}" data-party-id="${escapeHtml(unit.id)}" data-party-command="attack">通常攻撃</button>
            <button class="text-button battle-party-command ${cmd.action === 'skill' ? 'is-selected' : ''}" data-party-id="${escapeHtml(unit.id)}" data-party-command="skill">スキル</button>
            <button class="text-button battle-party-command ${cmd.action === 'burst' ? 'is-selected' : ''}" data-party-id="${escapeHtml(unit.id)}" data-party-command="burst" ${unit.gauge >= 100 ? '' : 'disabled'}>必殺技</button>
            <button class="text-button battle-party-command ${cmd.action === 'guard' ? 'is-selected' : ''}" data-party-id="${escapeHtml(unit.id)}" data-party-command="guard">防御</button>
          </div>
          ${cmd.action === 'guard' || unit.combat?.role === 'aoe_support' && (cmd.action === 'skill' || cmd.action === 'burst') ? '<p class="muted" style="margin-top:8px;">この行動はターゲット選択不要</p>' : renderTargetAssist(unit.id)}
          <p class="muted" style="margin-top:8px;">スキル：${escapeHtml(unit.combat?.skill?.description || '-')}</p>
        </div>
      `;
    }).join('')}
  `;
...
function attachBattleUiEvents() {
  document.querySelectorAll('[data-protagonist-command]').forEach((button) => button.addEventListener('click', () => { battleUiState.protagonistCommand = button.getAttribute('data-protagonist-command'); renderBattleUi(); }));
  document.querySelectorAll('[data-party-command]').forEach((button) => button.addEventListener('click', () => { const id = button.getAttribute('data-party-id'); const cmd = button.getAttribute('data-party-command'); if (!battleUiState.partyCommands[id]) battleUiState.partyCommands[id] = { action: 'attack', targetId: '' }; battleUiState.partyCommands[id].action = cmd; if (cmd !== 'guard') battleUiState.targetPickerMember = id; renderBattleUi(); }));
  document.querySelectorAll('[data-pick-target]').forEach((button) => button.addEventListener('click', () => { battleUiState.targetPickerMember = button.getAttribute('data-pick-target'); renderBattleUi(); }));
  document.querySelectorAll('[data-enemy-target]').forEach((button) => button.addEventListener('click', () => { const enemyId = button.getAttribute('data-enemy-target'); const memberId = battleUiState.targetPickerMember; if (!memberId || !battleUiState.partyCommands[memberId]) return; battleUiState.partyCommands[memberId].targetId = enemyId; renderBattleUi(); }));
  document.getElementById('battle-execute-turn')?.addEventListener('click', executeBattleTurn);
  document.getElementById('battle-reset')?.addEventListener('click', resetBattle);
  document.getElementById('battle-close')?.addEventListener('click', () => { battleUiState.currentBattleId = null; battleUiState.runtime = null; battleUiState.turnCount = 0; battleUiState.turnLogs = []; battleUiState.result = null; renderBattleUi(); });
}
function renderBattleUi() {
  ensureBattleFxStyles();
  const card = document.getElementById('battle-ui-card'); if (!card) return;
  const battle = getBattleById(battleUiState.currentBattleId); const runtime = battleUiState.runtime;
  if (!battle || !runtime) { card.innerHTML = '<h3>戦闘本実装 v1.2</h3><p class="muted">「バトルUIを開く」を押すと、ここに戦闘UIが表示されます。</p>'; return; }
  cleanBattleTransientEffects(runtime);
  card.innerHTML = `
    <h3>${escapeHtml(battle.title)}</h3>
    <p class="muted">戦闘本実装 v1.2：ダメージ数字 / 被弾アニメ / 必殺演出 / 勝利演出 / 敵HPバークリックターゲット指定 / 選択中敵の強調表示。</p>
    ${renderBattleBurstFx(runtime)}
    ${battleUiState.result?.status === 'victory' ? renderBattleVictoryFx(runtime) : ''}
    <div class="info-card" style="margin-top:12px;"><h4 style="margin-top:0;">味方ユニット</h4>${runtime.party.map((unit) => renderBattleUnitBox(unit, false)).join('')}</div>
    <div class="info-card" style="margin-top:12px;"><h4 style="margin-top:0;">敵ユニット（HPバークリックでターゲット選択）</h4>${runtime.enemies.map((unit) => renderBattleUnitBox(unit, true)).join('')}</div>
    ${renderBattleCommandControls(runtime)}
    <div class="info-card" style="margin-top:12px;"><h4 style="margin-top:0;">戦闘ログ</h4>${renderBattleLog()}</div>
    ${renderBattleResultPanel()}
    <div class="button-group" style="margin-top:18px;max-width:none;flex-direction:row;flex-wrap:wrap;">
      <button class="text-button" id="battle-execute-turn" ${battleUiState.result ? 'disabled' : ''}>この行動で1ターン進む</button>
      <button class="text-button" id="battle-reset">バトルをリセット</button>
      <button class="text-button" id="battle-close">閉じる</button>
    </div>
  `;
...
// -------------------------
// Story / Adventure / Formation
// -------------------------
function openStoryReader(storyId) { const story = (gameData.story?.stories || []).find((item) => item.id === storyId); if (!story) return; storyReaderState.currentStoryId = storyId; storyReaderState.currentSceneIndex = 0; renderStoryReader(); }
function renderStoryReader() {
  const readerCard = document.getElementById('story-reader-card'); if (!readerCard) return;
  const story = (gameData.story?.stories || []).find((item) => item.id === storyReaderState.currentStoryId);
  if (!story) { readerCard.innerHTML = '<h3>ストーリー本文ビューア</h3><p class="muted">「本文を読む」を押すと、ここに本文が表示されます。</p>'; return; }
  const scenes = story.scenes || []; const maxIndex = scenes.length - 1; const currentIndex = clamp(storyReaderState.currentSceneIndex, 0, Math.max(0, maxIndex)); storyReaderState.currentSceneIndex = currentIndex; const scene = scenes[currentIndex] || {};
  readerCard.innerHTML = `<h3>${escapeHtml(story.title)}</h3><p class="muted">${escapeHtml(story.id)} / ${currentIndex + 1} / ${Math.max(1, scenes.length)}</p><div class="status-row"><span>話者</span><strong>${escapeHtml(scene.speaker || 'ナレーション')}</strong></div><div class="status-row"><span>背景</span><strong>${escapeHtml(scene.bg || '未設定')}</strong></div><div class="info-card" style="margin-top:16px;"><p style="white-space:pre-wrap;margin:0;">${escapeHtml(scene.text || '')}</p></div><div class="button-group" style="margin-top:18px;max-width:none;flex-direction:row;flex-wrap:wrap;"><button class="text-button" id="story-prev-button">前へ</button><button class="text-button" id="story-next-button">次へ</button><button class="text-button" id="story-close-button">閉じる</button></div>`;
...
function renderAdventureSection() {
  const screen = document.getElementById('screen-adventure'); if (!screen || !gameData.story || !gameData.battles) return;
  const container = screen.querySelector('.screen-inner'); if (!container) return; clearDynamicCards(container);
  const stories = gameData.story.stories || []; const battles = gameData.battles.battles || []; const chapterTitle = gameData.story.meta?.chapterTitle || '第1章'; const characterMap = new Map(getCharactersList().map((char) => [char.id, char]));
  const nodeOrder = ['story_1_1', 'story_1_2', 'battle_1_1', 'story_1_3', 'battle_1_2', 'battle_1_3'];
  const storyMap = new Map(stories.map((i) => [i.id, i])); const battleMap = new Map(battles.map((i) => [i.id, i]));
  const resourceCard = createInfoCard(); resourceCard.innerHTML = renderResourceSummaryCard('現在の所持数');
  const timelineCard = createInfoCard(); timelineCard.innerHTML = `<h3>${escapeHtml(chapterTitle)}</h3><ol class="story-list">${nodeOrder.map((id) => { const story = storyMap.get(id); const battle = battleMap.get(id); if (story) { const unlockText = getStoryUnlockText(story, characterMap); return `<li><strong>ストーリー</strong>：${escapeHtml(story.title)} <span class="muted">(${escapeHtml(story.id)})</span><br>${unlockText ? `<span class="muted">${escapeHtml(unlockText)}</span><br>` : ''}<button class="text-button story-open-button" data-story-id="${escapeHtml(story.id)}">本文を読む</button></li>`; } if (battle) { const reward = `初回: コア ${battle.firstClearReward?.materialCore ?? 0} / EXP ${battle.firstClearReward?.exp ?? 0}`; const repeat = `再挑戦: EXP ${battle.repeatReward?.exp ?? 0}`; const cleared = progressState.clearedBattles.includes(battle.id) ? ' / クリア済み' : ''; return `<li><strong>バトル</strong>：${escapeHtml(battle.title)} <span class="muted">(${escapeHtml(battle.id)}${escapeHtml(cleared)})</span><br><span class="muted">${escapeHtml(reward)}</span><br><span class="muted">${escapeHtml(repeat)}</span><br><button class="text-button battle-preview-button" data-battle-id="${escapeHtml(battle.id)}">バトル情報</button> <button class="text-button battle-open-button" data-battle-id="${escapeHtml(battle.id)}">バトルUIを開く</button></li>`; } return `<li>${escapeHtml(id)}</li>`; }).join('')}</ol>`;
  const reader = createInfoCard('story-reader-card'); reader.innerHTML = '<h3>ストーリー本文ビューア</h3><p class="muted">「本文を読む」を押すとここに表示されます。</p>';
  const battleCard = createInfoCard('battle-ui-card'); battleCard.innerHTML = '<h3>戦闘本実装 v1.2</h3><p class="muted">「バトルUIを開く」を押すと、ここに戦闘UIが表示されます。</p>';
  container.appendChild(resourceCard); container.appendChild(timelineCard); container.appendChild(reader); container.appendChild(battleCard); attachAdventureButtons();
}
function getAvailableWeaponsForSlot(slotId) {
  return getWeaponsList().filter((weapon) => {
    if (!canEquipWeaponToSlot(weapon, slotId)) return false;
    const usage = getEquippedWeaponUsageMap(); const owned = getOwnedWeaponCount(weapon.id); const current = formationState.equippedWeapons[slotId] === weapon.id ? 1 : 0;
    return owned - ((usage.get(weapon.id) || 0) - current) > 0;
  });
}
function renderFormationSection() {
  const screen = document.getElementById('screen-formation'); if (!screen || !gameData.characters || !gameData.weapons) return;
  const container = screen.querySelector('.screen-inner'); if (!container) return; clearDynamicCards(container);
  const resourceCard = createInfoCard(); resourceCard.innerHTML = renderResourceSummaryCard('現在の所持数');
  const equipCard = createInfoCard(); const slots = ['protagonist', 'char_towa', 'char_hinano', 'char_suzu'];
  equipCard.innerHTML = `<h3>編成</h3>${slots.map((slotId) => { const current = formationState.equippedWeapons[slotId] || ''; const options = ['<option value="">未装備</option>', ...getAvailableWeaponsForSlot(slotId).map((weapon) => `<option value="${escapeHtml(weapon.id)}" ${weapon.id === current ? 'selected' : ''}>${escapeHtml(weapon.name)} [${escapeHtml(getWeaponTypeDisplay(weapon))} / 所持 ${escapeHtml(getOwnedWeaponCount(weapon.id))} / +${escapeHtml(getWeaponEnhanceLevel(weapon.id))}]</option>`)].join(''); const stats = computeFinalStatsForSlot(slotId, formationState.equippedWeapons); return `<div class="info-card" style="margin-top:12px;"><h4 style="margin-top:0;">${escapeHtml(getSlotDisplayName(slotId))}</h4><select data-equip-slot="${escapeHtml(slotId)}" style="width:100%;padding:10px;border-radius:12px;border:1px solid #2d3966;background:#0f1630;color:#eef2ff;">${options}</select><p class="muted" style="margin-top:8px;">最終HP ${escapeHtml(stats.finalHp)} / 最終ATK ${escapeHtml(stats.finalAtk)}</p></div>`; }).join('')}`;
  const enhanceCard = createInfoCard(); enhanceCard.innerHTML = `<h3>武器強化</h3>${getWeaponsList().filter((weapon) => getOwnedWeaponCount(weapon.id) > 0).map((weapon) => { const ew = getEffectiveWeaponStats(weapon); const nextCost = getWeaponEnhanceCost(ew.level); return `<div class="info-card" style="margin-top:12px;"><strong>${escapeHtml(weapon.name)}</strong> <span class="muted">(+${escapeHtml(ew.level)} / 所持 ${escapeHtml(getOwnedWeaponCount(weapon.id))})</span><br><span class="muted">HP ${escapeHtml(ew.hp)} / ATK ${escapeHtml(ew.atk)}</span><br><button class="text-button" data-weapon-enhance="${escapeHtml(weapon.id)}" ${canEnhanceWeapon(weapon.id) ? '' : 'disabled'}>強化する（${escapeHtml(nextCost)} コア）</button></div>`; }).join('') || '<p class="muted">所持武器なし</p>'}`;
  container.appendChild(resourceCard); container.appendChild(equipCard); container.appendChild(enhanceCard); attachFormationEvents();
}
function attachFormationEvents() {
  document.querySelectorAll('[data-equip-slot]').forEach((select) => select.addEventListener('change', () => { formationState.equippedWeapons[select.getAttribute('data-equip-slot')] = select.value; sanitizeEquippedWeapons(); saveGameState(); renderFormationSection(); }));
  document.querySelectorAll('[data-weapon-enhance]').forEach((button) => button.addEventListener('click', () => tryEnhanceWeapon(button.getAttribute('data-weapon-enhance'))));
}
...
function setupNavigation() {
  document.querySelector('[data-action="start"]')?.addEventListener('click', () => showScreen('menu'));
  document.querySelector('[data-action="load"]')?.addEventListener('click', () => {
    const loaded = loadSaveData();
    if (loaded) {
      sanitizeOwnedWeapons(); sanitizeWeaponEnhancements(); sanitizeProtectedWeapons(); sanitizeEquippedWeapons(); rerenderAllSections(); alert('保存済みデータを読み込みました。'); showScreen('menu');
    } else { alert('保存データがありません。'); }
  });
  document.querySelector('[data-action="settings"]')?.addEventListener('click', () => alert('設定画面は後続実装予定です。'));
  document.querySelectorAll('[data-screen]').forEach((button) => button.addEventListener('click', () => showScreen(button.getAttribute('data-screen'))));
  document.querySelectorAll('[data-back]').forEach((button) => button.addEventListener('click', () => showScreen(button.getAttribute('data-back'))));
}
async function bootstrapGameData() {
  try {
    const [storyRes, charRes, battleRes, weaponRes] = await Promise.allSettled([loadJson('story.json'), loadJson('characters.json'), loadJson('battles.json'), loadJson('weapons.json')]);
    gameData.story = storyRes.status === 'fulfilled' ? storyRes.value : { meta: { chapterTitle: '第1章' }, stories: [] };
    gameData.characters = charRes.status === 'fulfilled' ? charRes.value : { meta: {}, characters: [] };
    gameData.battles = battleRes.status === 'fulfilled' ? battleRes.value : { meta: {}, battles: [], enemyTemplates: [] };
    gameData.weapons = weaponRes.status === 'fulfilled' ? weaponRes.value : { meta: {}, weapons: [] };
    loadSaveData();
    return true;
  } catch (error) {
    console.error(error); renderLoadError(error); return false;
  }
}
function renderLoadError(error) {
  ['screen-adventure', 'screen-formation', 'screen-gacha'].map((id) => document.getElementById(id)).forEach((screen) => {
    if (!screen) return; const container = screen.querySelector('.screen-inner'); if (!container) return; clearDynamicCards(container);
    const card = createInfoCard(); card.innerHTML = `<h3>データ読み込みエラー</h3><p class="muted">${escapeHtml(error.message)}</p><p class="muted">ローカルサーバーまたは Cloudflare Pages 上で確認してください。</p>`; container.appendChild(card);
  });
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  bootstrapGameData().then((ok) => {
    if (!ok) return;
    initializeStarterWeaponsIfNeeded(); sanitizeOwnedWeapons(); sanitizeWeaponEnhancements(); sanitizeProtectedWeapons(); sanitizeEquippedWeapons(); saveGameState(); rerenderAllSections();
  });
});
